...
public class RestaurantServiceTest extends ServiceTest<RestaurantInteractor, AuthenticatedRestaurantRequests,
        UnauthenticatedRestaurantRequest> {

    public RestaurantServiceTest() {
        super(RestaurantInteractor.class, AuthenticatedRestaurantRequests.class, UnauthenticatedRestaurantRequest.class);
        OuiLogger.log(Level.DEBUG, "Testing Restaurant Service Requests");
    }

    @AfterAll
    public void teardown() {
        OuiLogger.log(Level.DEBUG, "Finished Testing Restaurant Service Requests");
    }
    ...
    public void getRestaurantsByNameSuccess() {
        when(interactor.findRestaurantByName("name")).thenReturn(new ArrayList<>(List.of(new Restaurant("name", "address", "phone", "website", 14, new Category[]{new Category("asd", "ad")}, 3.2, new Coordinates(1, 2), "$", new Location("city", "state", "country", "zip", "address", "address2", "address3", new String[]{"address4"}), "description", "hours"))));
    }
